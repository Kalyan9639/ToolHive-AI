...
class AI_Tool_Agent:
    def __init__(self, start_url, output_file="extracted_ai_tools.csv", interval_seconds=3600):
        ...
        # --- CSV Initialization & Automatic Migration ---
        self.columns = ['Title', 'Slug', 'Category', 'Description', 'Link', 'Logo', 'Scraped_At']
        
        if os.path.exists(self.output_file):
            try:
                df = pd.read_csv(self.output_file)
                # Check if 'Slug' column is missing (Migration Logic)
                if 'Slug' not in df.columns:
                    print(f"[Scraper] Migrating {self.output_file} to include 'Slug' column...")
                    # Generate slugs for existing titles
                    df['Slug'] = df['Title'].apply(self.create_slug)
                    ...
                    print("[Scraper] Migration complete. CSV structure updated.")
            except Exception as e:
                print(f"[Scraper] Warning: Could not validate existing CSV: {e}")
        else:
